...
//			today = sdf.parse(new Date());
	...
	private static void duplicateValidate(ArrayList<Customer> customers,String email) throws CustomerHandlingException {
//		if(emailValidate(email)) {
	...
	public static Customer validateCust(String name, String email, String password, double registrationAmount, String dateOfBirth, String type, ArrayList<Customer> customers) throws CustomerHandlingException, ParseException {
		duplicateValidate(customers, email);
		passwordValidate(password);
		Date d1 = parseAndValidate(dateOfBirth);
		double age = ((today.getTime()-d1.getTime())/31556952)/1000;
//		if(age>60)
//			CustomerType type2 = CustomerType.PLATINUM;
//		CustomerType type1 = valCustomerType(type,registrationAmount);
//		CustomerType type1;
		CustomerType type1 = (age>60)?CustomerType.PLATINUM:valCustomerType(type,registrationAmount);
//		System.out.println(((today.getTime()-d1.getTime())/31556952)/1000);
		return new Customer(name, email, password, registrationAmount, d1, type1);
		
	}
	
	//customer login
	public static boolean customerLoginPrompt(String email, String password, ArrayList<Customer> customers) throws CustomerHandlingException{
		int count = 0;
		if(email.matches(".*[@]*.com$")) {
			for(Customer c : customers) {
				if(c!=null) {
	...
	public static Customer validEmail(String email, ArrayList<Customer> customers) throws CustomerHandlingException {
		if(email.matches(".*[@]*.com$")) {
			for(Customer c : customers) {
				if(c!=null) {
