...
public class Showroom {
	...
	public static void main(String[] args) {
		try (Scanner sc = new Scanner(System.in)) {
			...
			boolean exit = false;
			while (!exit) {
				System.out.println("Options 1. Add Vehicle 2.Display all 3. Purchase a Vehicle Exit");
				try {
					switch (sc.nextInt()) {
					case 1:
					...
					case 2: //display 
						...
						break;
					case 3://purchase a vehicle
						System.out.println("Enter the chasis no.");
						//onvoke the validation method to check if vehicle by specified chasis no exists or not
						int chasisNo = sc.nextInt();
						Vehicle v = findByChasisNo(chasisNo,vehicles);
						//=> vehicle is found, now check if it is available
						if(v.getAddress() == null) {
							...
							throw new VehicleHandlingException("Vehicle with chasis no."+chasisNo +" already sold!!!");
