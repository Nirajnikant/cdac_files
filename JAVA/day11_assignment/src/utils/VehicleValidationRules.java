package utils;

import java.text.ParseException;
import java.util.ArrayList;
import java.util.Date;

import com.app.vehicles.Category1;
import com.app.vehicles.Vehicle;

import custom_exception.VehicleHandlingException;
import static com.app.vehicles.Vehicle.sdf;

public class VehicleValidationRules {
	public static Date startDate, endDate;
	static {
		try {
			startDate = sdf.parse("1-4-2021");
			endDate = sdf.parse("31-3-2022");
		} catch (ParseException e) {
			System.out.println("err in static init block " + e);
		}
	}

//add public static method to chk for dup chasis no
	public static void checkForDup(ArrayList<Vehicle> vehicles, int newChasisNo)// pass by value i.e a copy is made
			throws VehicleHandlingException {
		Vehicle newVehicle = new Vehicle(newChasisNo);
//		for (Vehicle v : vehicles)// uses Iterator interally
//			if (v != null)
//				if (v.equals(newVehicle))// int ---> Integer ---> Object
//					// =>dup detected --> throw cust exc
//					throw new VehicleHandlingException("Dup chasis no detected!!!!!");
//		// =>no dups
//		System.out.println("no dup detected!!!!");
		if(vehicles.contains(newVehicle))
			throw new VehicleHandlingException("Dup chasis no detected!!!!!");
		System.out.println("no dup detected!!!!");
	}

	// add public static method for category validation
	public static Category1 validateCategory(String category) {
		return Category1.valueOf(category.toUpperCase());// if user supplies unknown category : JVM will throw
//		java.lang.IllegalArgumentException : uncheckeed exception
	}

	// add static method :to validate manufactureDate : in this financial year (1st
	// Apr 2021 to 31Mar 2022)
	public static Date parseAndValidateDate(String date) throws ParseException,VehicleHandlingException {
		// parsing
		Date d1 = sdf.parse(date);
		// => parsing success , proceed to validation
		if(d1.before(startDate) || d1.after(endDate)) //=> err
			throw new VehicleHandlingException("Invalid date!!!!!");
		//=> validations success
		return d1;
	}

	// add static method to invoke all validation rules n throw exc in case of
	// failures or ret Vehicle instance to the caller
	public static Vehicle validateAllInputs(int chasisNo, String color, 
			double price, String manufactureDate,String category,ArrayList<Vehicle> showroom) throws VehicleHandlingException,ParseException
	{
		checkForDup(showroom, chasisNo);
		Date d1=parseAndValidateDate(manufactureDate);
		Category1 validatedCategory = validateCategory(category);		
		//=> all i/ps are valid
		return new Vehicle(chasisNo, color, price, d1, validatedCategory);	
	}
	
	//add a static method to check if vcehicle exists by chasis no. : finder method : findByChasisNo
	public static Vehicle findByChasisNo(int chasisNo, ArrayList<Vehicle> vehicles) throws VehicleHandlingException {
		Vehicle v1 = new Vehicle(chasisNo);
//		for(Vehicle v : vehicles) {
//			if(v!=null)
//				if(v.equals(v1)) //=> found :: int --> Integer --> Object
//					return v;
//		}
//		throw new VehicleHandlingException("Invalid chasis no!!!");
		if(vehicles.indexOf(v1)==-1)//O(n) : time complexity
			throw new VehicleHandlingException("Invalid chasis no!!!");
		return vehicles.get(vehicles.indexOf(v1));//O(1) : time complexity
	}
	
	//Update Vehicle price
	public static Vehicle updatePrice(int chasisNo, double price, ArrayList<Vehicle> vehicles) throws VehicleHandlingException {
		Vehicle v = findByChasisNo(chasisNo, vehicles);
		v.setPrice(price);// we use setter instead of ArrayList set method because set method sets the reference in the ArrayList
		//rather than making changing the state of Vehicle
		return v;
	}
	
	//delete Vehicle
//	public static void deleteVehicle(int chasisNo, ArrayList<Vehicle> vehicles) throws VehicleHandlingException {
////		Vehicle v = findByChasisNo(chasisNo, vehicles);
////		vehicles.remove(v);
//		Vehicle v = new Vehicle( sc.nextInt());
//	}
//	
	//applying discount
	public static void discount(String date, String category, ArrayList<Vehicle> vehicles) throws ParseException, VehicleHandlingException {
		Category1 c = validateCategory(category);
		Date d1 = parseAndValidateDate(date);
		for(Vehicle v : vehicles) {
			if(v.getManufactureDate().before(d1) && v.getCategory()==c) {
				v.setPrice(v.getPrice()-v.getPrice()/10);//10% discount
			}
		}
	}
	
	//purchase 
	public static Vehicle purchase(int chasisNo, ArrayList<Vehicle> vehicles) throws VehicleHandlingException {
		Vehicle v = findByChasisNo(chasisNo, vehicles);
		if(v.getAddress()!=null)
			throw new VehicleHandlingException("Vehicle is OUT OF STOCK!!!");
		return v;
	}
	
	//findbycity
	public static String findByCity(String city, ArrayList<Vehicle> vehicles) throws VehicleHandlingException {
		String result = null;
		int count = 0;
		for(Vehicle v : vehicles) {
			if(v.getAddress().getCity().equals(city)) {
				result += "chasis no:"+v.getChasisNo()+" price : "+v.getPrice();
				count++;
			}
		}
		if(count==0)
			throw new VehicleHandlingException("No vehicle delivered to "+city);
		return result;
	}
	
}
