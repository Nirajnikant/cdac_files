...
// but if the instance variable name is same as that of the mlv then ****this must be used with instqance variable	
	...
	String getBoxDetails() {
		...
		return "Box dimensions " + this.width + " " + this.depth + " " + this.height + this;
	}
	...
	Box createNewBox(double wOffset,double dOffset,double hOffset) {
		Box temp = new Box(width += wOffset,depth += dOffset,height += hOffset);
		return temp;
	}
	
	Box(double w, double d, double height, String c) {
		this (w,d,height);
		this.color = c;	
	}
	String getColor() {
